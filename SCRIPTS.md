...
4. 按 `.env APP_ENV=production` 守卫把 `composer.production.json` 临时覆盖到 `composer.json`（切换前备份，Composer 失败时回滚）
...
| `cache.sh` | 本地层 — 定向清框架生成物（不清业务 cache）+ `optimize` / 目录权限 / daily log 治理 |

> ⚠️ cache.sh **不跑** composer。vendor 不在 → 报错让你走 pull.sh。
> 它也不运行 `cache:clear` / `optimize:clear`，避免清掉 Redis 中的 JWT 黑名单。
...
**为什么 cron 要每天跑**：预创建只覆盖今天+明天，不每天跑则后天 00:00 又回到 race。
首次部署 / 属主异常先人工跑一次 `sudo sh cache.sh`；日常再交给 web user crontab：
`55 23 * * * /bin/sh /path/to/repo/cache.sh >/dev/null 2>&1`（非 tty 自动不刷 tip）。
...
`DB_DATABASE`/`DB_USERNAME`/…。默认 `--ignore-table-data` 跳过 `system_operation_logs` 的数据，
保留表结构；密码只通过单次命令的 `MYSQL_PWD` 环境变量传入，不出现在进程参数中。
...
```bash
bash tools/fix-nginx-storage-safe.sh --dry-run                     # 沙盘，只看会改什么
bash tools/fix-nginx-storage-safe.sh                               # 实改（自动备份）
bash tools/fix-nginx-storage-safe.sh --dir /etc/nginx/conf.d       # 指定 vhost 目录
bash tools/fix-nginx-storage-safe.sh --exts 'php[0-9]*|phtml|phar' # 自定义后缀黑名单
```

脚本依赖 Bash 语法；不要用 `sh tools/fix-nginx-storage-safe.sh` 绕过它的 Bash shebang。
...
55 23 * * * /bin/sh /path/to/repo/cache.sh >/dev/null 2>&1
```
