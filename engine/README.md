...
- `scaffold/` 是 moo-scaffold 代码生成器的配置与 YAML 真值源；`composer.json` 是当前可运行配置，`composer.production.json` 是目标生产配置样例（开源包走 Packagist，`moo-system` / `moo-upload` 走私有 VCS；差异见教程第 2、8 章）。
