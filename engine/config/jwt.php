...
return [
    ...
    */
    ...
    |
    | 为什么默认写死 true（决策 why，坑 #28）：moo-system 的「登录管理·撤销会话」
    | 与「改密/禁用即踢下线」全部走 JWT 黑名单落地——撤销 = 把该 token forceForever
    | 拉黑。本键一旦为 false，被拉黑的 token 会被 decode 静默放行，撤销/踢人接口
    | 返回 200「看似成功、实际无效」，被撤销者继续畅通无阻，且没有任何报错线索，
    | 排查成本极高（宿主项目实战踩过：改密后旧会话仍能操作）。故此处不接受关闭，
    | env 仅留作应急旋钮，缺省即 true；行为由 AuthTest 的黑名单用例守护。
    |
    */
