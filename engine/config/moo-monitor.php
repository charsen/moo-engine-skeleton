...
/*
 * moo-monitor-laravel 配置（host 调优基线，1-4 发布）。
 *
 * 包自带默认值，host 不发布也能跑；发布后可在此调旋钮：
 *   php artisan vendor:publish --tag=moo-monitor-config
 *
 * 路径约定：绝对路径原样使用；相对路径挂在 storage_path() 下（该目录自带 .gitignore，不入 host git）。
 * 运行时异常/慢 SQL 先落本地 YAML 缓冲，再由调度批量推送云端（见 routes/console.php 与 cloud 段）。
 */
return [
    ...
    'runtime' => [
        ...
        // open 桶最大条数（写盘闸：超过后新 hash 不再落盘，防异常洪峰打爆磁盘）
        ...
        // 命中的值在落盘前被掩码，避免把密码/token 写进监控 yaml。
        'mask_keys' => ['password', 'pwd', 'token', 'secret', 'api_key', 'authorization'],

        // payload 单字段截断长度
        'string_truncate' => 200,

        // trace 字段最大字节数（超长堆栈截断，防单条 yaml 膨胀）
    ...
    'exception' => [
        ...
        // false 时退回 host 在 bootstrap/app.php 手动接入（两者并存不会双计）。
        'auto_hook' => true,

        // php -r / tinker 里的实验异常跳过（不是真实请求，别污染监控）
    ...
    'sql_slow' => [
        ...
        // SQL 子串命中即跳过（高频噪音：操作日志写入、迁移表探测等）
        'skip_patterns' => [
    ...
    'cloud' => [
        ...
        // enabled + schedule 同时为真时包自动挂每分钟调度（需 host 跑 schedule:run）
        'schedule' => (bool) env('MOO_MONITOR_CLOUD_SCHEDULE', true),

        // 推送成功后本地回收阈值（天）：resolved 全清，open 清 last_seen 超过 N 天的；<=0 不回收
