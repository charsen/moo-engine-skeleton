...
| `composer.production.json` | 生产部署 | `vcs`（git clone 锁 `composer.lock` 版本） | 装成实体目录、版本可复现 |
...
| 脚本 | 职责 |
| --- | --- |
| `pull.sh` | 网络层：git pull + 验证私包权限（ssh + ls-remote）+ 切 `composer.production.json` + `composer install/update` 私包 + `vendor:publish` + 调 cache.sh |
...
### 3.1 生产侧 `composer.json` 持久 "M" 状态（预期，不是 bug）

首次跑完 `sudo sh pull.sh` 后，服务器 `git status` 会**长期显示 `M engine/composer.json`**——因为
pull.sh Step 4 把 `composer.production.json` 内容 cp 覆盖了 `composer.json`。

- 服务器上 `composer.json` 实际内容是 production 版（vcs），这是对的
- 每次 deploy 又重新 cp 一次（production.json 没变则 `diff -q` 相同 → 跳过 cp）
- pull.sh Step 1 工作区检查**已自动过滤** `M engine/composer.json` 这一条，不会 abort

> **生产侧绝不要** `git commit engine/composer.json`——那会把 production 配置 commit 回主仓、污染开发版。
...
- `composer.production.json` 真覆盖了 `composer.json`？（`diff engine/composer.json engine/composer.production.json` 应为空）
- 是不是本地误跑了 cache.sh 把 composer.json 覆盖回去了 → `git checkout engine/composer.json`
...
| 坑 | pull.sh 已防 |
| --- | --- |
| composer.json 持续 `M` 脱状（Step 4 cp 产物） | Step 1 grep 过滤该条 |
