...
### 统一上传不能让公开骨架硬依赖私有基础包
- **日期**：2026-08-12
- **症状**：多个扩展包各自实现上传 intent、临时目录和对象存储驱动，Host 同时出现两套引用格式、消费状态和失败清理；若直接把私有上传包写进公开骨架，又会让无授权开发者无法安装。
- **根因**：把“业务包需要一个媒体字段”与“上传基础设施由谁拥有”混在一起，没有为公开包保留独立运行边界。
- **解法**：公开业务包只定义窄媒体契约和默认 Filesystem 实现；私有 Host 安装 `moo-upload` 后在自身 Provider 绑定适配器。上传 intent/reference、purpose 与厂商接入只留一套，业务包仍拥有业务归属、发布状态和删除语义。
