...
### 统一上传需区分公开包与私有整套骨架
- **日期**：2026-08-12
- **症状**：多个扩展包各自实现上传 intent、临时目录和对象存储驱动，Host 同时出现两套引用格式、消费状态和失败清理；私有 `moo-system` 仍要求每个 Host 复制头像适配器与 purpose。
- **根因**：把“公开业务包必须独立可安装”机械套到私有 Moo 整套体系，导致私有业务包的共用能力反而散落 Host。
- **解法**：公开业务包只定义窄媒体契约和默认 Filesystem 实现；私有业务包直接依赖 `moo-upload`，在包内注册 namespaced purpose、适配器与 verifier。公开骨架的前六章仍不依赖私包，选择第七章/完成态私有体系时同时取得授权并安装 `moo-system`、`moo-upload`；Host 只保留安全组、ACL/API 元数据和环境配置。
