...
见 `HANDOFF.md` §6）；且本仓库的 composer path 依赖要求本地有它的克隆才能装起来
（见下文「克隆后首次跑通」的前置说明）。
...
统一的目录约定。
仓库根目录只放文档、CI 配置和本文件（没有部署脚本——部署是 docs 第 8 章的指引）。

```
moo-engine-skeleton/
├── CLAUDE.md  README.md                 # 本指引 + 仓库说明（仓库根）
...
**前置（致命）**：`engine/composer.json` 走 path 仓库，`composer install` 硬性要求上级目录下
**同级**存在 `moo-scaffold` 与 `moo-system` 的克隆（两者目前均为 Gitee 私有仓库，
moo-system 需作者授权）——没有访问权限则第一步就失败，目前没有公开替代源。
克隆布局与新机环境（brew php@8.3、PATH 处理）见 **`HANDOFF.md` §1~§2**。
...
**`engine/composer.json` 已按「开发（path）」配好**——下面两段 JSON 是给你对照理解的，
不需要动手改；只有部署生产时才按第二段切换。
...
然后**必须 `composer update` 重锁**——仓库里的 `composer.lock` 是按 path 仓库解析的，
与 vcs require 冲突，直接 `composer install` 会卡住。
...
#   moo:adder 是交互式命令；folder 参数必须带尾斜杠（写 Food 会直接崩，坑 #24）——完整走法见 docs 第 9.4 节
```
...
- **boot 前必须提供的 host 契约**——**本骨架已全部实现**（`moo-system check` 6/6 通过，
...
> 以上 moo-scaffold / moo-system 的文档与源码都在**同级克隆的私有仓库**里
> （见「克隆后首次跑通」的前置）——只克隆了本仓库的读者打不开这些路径。

## 搭建进度 —— 已完成（2026-06 教学路线重构后）

全部十章搭好并真机验证（第 8 章为部署指引，代码产物即根目录的 `.github/workflows/tests.yml`；
第 9 章为增量开发演练，有代码产物；第 10 章为云端监控进阶，纯文档指引）；从 0 开始的过程写在 `docs/`
（`docs/README.md` 是目录，含一张 27 条「踩过的坑」速查表；`docs/index.html`
...
6. ✅ 移动端 `Api/` 分片：user 守卫主体是自建 User（email 登录，**永久**）；
   ...
   `forceForever=true` 把旧 token 永久拉黑（单设备语义，无 90s 宽限）、`resetClaims=false`。
...
全量测试 43 个（AuthTest/FoodAclTest/ApiAuthTest 三件套 + JwtAutoRefresh/SeederIntegrity/Regression 守护测试 + FoodIncremental/ApiFood + MonitorTest + 示例），
