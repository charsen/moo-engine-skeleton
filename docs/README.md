...
| moo-monitor-laravel 源码 | dev-master | **非可选**：`engine/composer.json` 把 `charsen/moo-monitor-laravel` 也作为硬 `require`（path 仓库 `../../moo-monitor-laravel`，别名 `dev-master as 0.1.99`）。缺这个同级克隆，`composer install` 会因 path 找不到而直接失败。**获取方式**：`git clone https://gitee.com/charsen/moo-monitor-laravel.git`（当前私有，需联系作者申请协作者权限），克隆到同级 `wwwroot/moo-monitor-laravel/` |
