...
`moo-scaffold` 没有发布到 Packagist，是放在 Gitee 的私有包。接入前要先在
`engine/composer.json` 里声明 `repositories`。两种模式按环境选：
...
生成的后台控制器路由用了 `Route::iResource(...)` 宏，要在
`app/Providers/AppServiceProvider.php` 的 `boot()` 里注册（比 `Route::resource`
...
- `moo-scaffold` 以 path 模式接入，20 个 `moo:*` 命令可用；
