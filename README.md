...
# （需要 Gitee SSH 有 moo-scaffold 仓库访问权）
...
本骨架依赖作者的另外几个包。当前这些私有包都通过 Composer **VCS 仓库**接入：
`moo-scaffold`、`moo-monitor-laravel`、`moo-system` 都不走本地 path，也不需要 clone 到同级目录。
...
# 配好 Gitee SSH key，并确保账号/部署 key 有以下仓库访问权：
# git@gitee.com:charsen/moo-scaffold.git
# git@gitee.com:charsen/moo-monitor-laravel.git
# git@gitee.com:charsen/moo-system.git   # 第 7 章商业包，可选
```
...
这些包当前尚未发布到 Packagist，直接 `composer require charsen/<package>` 会找不到包；
必须先在 `composer.json` 的 `repositories` 里声明对应 Gitee VCS 仓库，再 require 版本约束。
...
> ⚠️ **前置**：
> ① 需 **PHP 8.2+**；
> ② 配好 Gitee SSH key，并确保有 **moo-scaffold**、**moo-monitor-laravel** 仓库访问权；
...
```bash
cd engine
composer install                                   # 通过 Gitee VCS 拉取 moo-* 私有包
...
#    先确保 Gitee SSH 有仓库访问权，再往 engine/composer.json 里加：
#      "repositories": { "scaffold": { "type": "vcs", "url": "git@gitee.com:charsen/moo-scaffold.git" } }
...
moo-engine-skeleton/
├── README.md                # 本文，仓库入口
├── HANDOFF.md               # 换机/交接速查（VCS 私有包 + 初始化清单）
├── overview.md              # 立项说明
├── CLAUDE.md                # AI 协作约定
...
| [第 8 章 部署上线（可选）](./docs/08-部署上线.md) | Composer VCS 部署、Redis、nginx、supervisor、清缓存坑 |
